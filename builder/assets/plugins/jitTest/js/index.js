...
var Log = {
  ...
  write: function(text){
    if (!this.elem) 
      ...
      this.infovis = $('#infovis');
    this.elem.innerHTML = text;
    this.elem.style.left = (this.infovis.offset().left + this.infovis.width()/2 - this.elem.offsetWidth / 2) + 'px';
  }
};
...
function init(js,fn){
    ...
    //end
    var jj = js || {};
    var nodeHeight = jj.nodeHeight;
    var nodeWidth = jj.nodeWidth;
    var nodeBgColor = jj.nodeBgColor;
    // 是否拖拽
    var panning = !!jj['panning'] ? false : true;
    ...
    var st = new $jit.ST({
        ...
        Node: {
            ...
            // 节点背景类型circle, ellipse
            type: 'ellipse',
            color: nodeBgColor,
            align:"center",
            overridable: true
        },
        Edge: {//线条类型(bezier,line,arrow,quadratic:begin,quadratic:end)样式设置 
            type: 'bezier',
            lineWidth: 2,
            color:'#23A4FF',
            overridable: true
    ...
    st.compute();
    //optional: make a translation of the tree整体渲染
